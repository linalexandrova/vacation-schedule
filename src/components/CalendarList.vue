        ...
        <calendar-item>
            ...
            <template #calendar-item-body>
                <div v-if="month.days && month.days.length > 0" class="calendar-list__list">
                    <div v-for="n in getEmptyDaysCount(month.days[0].week_day)" :key="'empty-' + n" class="calendar-list__list-item calendar-list__list-item-empty"></div>
    ...
    methods: {
        async fetchDataForAllMonths() {
            ...
            for (let i = 0; i < this.months.length; i++) {
                try {
                    const response = await axios.get(`https://production-calendar.ru/get-period/9e9ddd55d4f208e93c9e26ebeb9bd8e6/ru/${this.months[i]}.2024/json`);
                    tempDaysByMonth[i] = response.data as MonthData;
                } catch (error) {
                    console.error(`Error fetching data for month ${this.months[i]}`, error);
                }
            }
            this.daysByMonth = tempDaysByMonth; // Обновляем состояние одним разом после получения всех данных
